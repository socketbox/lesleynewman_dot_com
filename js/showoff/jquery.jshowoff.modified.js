...
(function($) {
	...
	$.fn.jshowoff = function(settings) {
		...
		this.each(function(i) {
			...
			if(config.hoverPause && !config.clickStop)
            {
                $cont.hover( function(){ if(isPlaying()) pause('hover'); },
			...
			function goToAndPause(index)
            {
			    ...
			    if( config.clickStop && (counter != index || ( (counter == index) && isBusy() ) ) )
                {
                ...
                else if((counter != index) || ((counter == index) && isBusy())){
					if(isBusy()) $cont.children().eq(0).remove();
					transitionTo(gallery,index);
					pause();
				};
			...
			function addSlideLinks() {
				...
				$.each(gallery, function(i, val)
                {
					...
					$('<a class="jshowoff-slidelink-'+i+' '+uniqueClass+'-slidelink-'+i+'" href="#null"></a>').bind
                        ('click', {index:i}, function(e){ goToAndPause(e.data.index); return false; }).appendTo
                        ('.'+uniqueClass+'-slidelinks');
				});
			};		
